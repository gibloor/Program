...
echo "<span class='td'> Login </span><span class='td'>Название книги</span><span class='td'>Как выдана</span><span class='td'> кнопочки </span>";
    foreach ($pageData['results'] as $key => $value) {
      echo "<form class='tr' method='post'>
      <span class='td'> " . $value['login'] . " </span><span class='td'> ";
      echo '<select name=\'name\'>';
    foreach ($pageData['resultz'] as $keys => $values) {
      echo '<option> '.$values['name'].' </option>';
    }
    echo "</select>";
    echo"</span><span class='td'><select name='typeissue'>
